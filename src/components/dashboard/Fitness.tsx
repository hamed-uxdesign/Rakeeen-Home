  ...
  return (
    <div className="max-w-2xl mx-auto py-12 px-6 min-h-screen">
      ...
      <Tabs 
        ...
        className="flex-wrap mb-12"
        size="md"
      />

      {tab === 'diet' && (
        <div className="animate-scale-in space-y-8">
          <div className="sys-card p-12 text-center relative overflow-hidden">
             ...
             <Label className="text-xs opacity-30 font-black tracking-[0.3em] mb-4 uppercase">Energy Consumed</Label>
             <div className="text-[80px] font-black text-ink tracking-tighter leading-none">
                {totalToday} <span className="text-2xl text-ink/10 font-normal ml-2">/ {CALORIE_GOAL}</span>
             </div>
          </div>
          <div className="sys-card p-10">
            <h3 className="text-xs font-black uppercase tracking-[0.3em] mb-10 opacity-30">Log Meal</h3>
            ...
            <div className="flex gap-6 items-end">
              <div className="flex-1">
                <Input label="Amount (Grams)" placeholder="100" type="number" value={grams} onChange={e => setGrams(e.target.value)} className="h-16 text-xl" />
              </div>
              <Button variant="premium" onClick={addMealItem} className="h-16 px-10"><HugeiconsIcon icon={PlusSignIcon} size={24} /></Button>
            </div>
          </div>
          <div className="space-y-6">
            {Object.entries(meals).map(([mealName, items]) => items.length > 0 && (
              <div key={mealName} className="sys-card p-10">
                <div className="flex justify-between items-center mb-8">
                  <h4 className="text-lg font-black uppercase tracking-[0.2em] text-forest">{mealName}</h4>
                  <span className="text-xs font-black text-ink/20 uppercase tracking-widest">{items.reduce((a, i) => a + i.kcal, 0)} kcal</span>
                </div>
                <div className="space-y-6">
                  {items.map((item) => (
                    <div key={item.id} className="flex justify-between items-center group">
                      <div>
                        ...
                        <div className="text-xs font-black opacity-20 uppercase tracking-[0.2em]">{item.grams}g · {item.time}</div>
                      </div>
                      <div className="flex items-center gap-6">
                        <span className="text-2xl font-black text-ink/70">{item.kcal} <span className="text-xs opacity-30 uppercase ml-1">kcal</span></span>
                        <button onClick={() => deleteMealItem(mealName, item.id)} className="opacity-0 group-hover:opacity-100 text-rust transition-all p-2 hover:bg-rust/10"><HugeiconsIcon icon={Delete02Icon} size={20} /></button>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      {tab === 'challenge' && (
        <div className="animate-scale-in space-y-8">
          <div className={`sys-card p-12 text-center ${isChallengeDone ? 'bg-forest/5 border-2 border-forest' : ''}`}>
             <div className="mb-12">
                <h3 className="text-4xl font-black text-ink mb-2 tracking-tighter">
                  {isChallengeDone ? 'HABIT SECURED' : 'No Sugar'}
                </h3>
                <p className="text-xs font-black uppercase tracking-[0.4em] text-forest opacity-60">
             ...
             {!isChallengeDone ? (
               <>
                 <div className="grid grid-cols-7 gap-4 mb-12 max-w-md mx-auto">
                    {Array.from({ length: 21 }).map((_, i) => (
                      <div 
                        key={i} 
                        className="aspect-square border-2 transition-all duration-700 flex items-center justify-center"
                        style={{ 
                 ...
                 <div className="text-center mb-12">
                    <div className="text-[100px] font-black text-forest leading-none tracking-tighter">{sugarChallenge.completedDays}</div>
                    <div className="text-xs font-black text-ink/20 uppercase tracking-[0.3em] mt-4">Days Completed / 21</div>
                 </div>
                 <div className="flex gap-4">
                    <Button variant="premium" onClick={checkInSugar} disabled={isTodayChecked} className="flex-1 h-20 !bg-forest !text-paper !border-forest text-lg tracking-widest font-black uppercase">{isTodayChecked ? 'Day Secured' : 'Log Success'}</Button>
                    {isTodayChecked && <button onClick={undoSugar} className="px-8 border-2 border-rust text-rust hover:bg-rust/10 transition-all font-black uppercase text-xs tracking-widest">Undo</button>}
                 </div>
               </>
             ) : (
               <div className="py-10">
      ...
      {tab === 'database' && (
        <div className="animate-scale-in space-y-8">
          <div className="sys-card p-10">
            <h3 className="text-xs font-black uppercase tracking-[0.3em] mb-10 opacity-30">Define New Food</h3>
            <div className="space-y-6">
              <Input label="Label" placeholder="e.g. Avocado" value={newFoodName} onChange={e => setNewFoodName(e.target.value)} className="h-16 text-xl" />
              <div className="grid grid-cols-2 gap-6">
                <Input label="Base (Grams)" type="number" placeholder="100" value={newFoodWeight} onChange={e => setNewFoodWeight(e.target.value)} className="h-16 text-xl" />
                <Input label="Energy (Kcal)" type="number" placeholder="160" value={newFoodKcal} onChange={e => setNewFoodKcal(e.target.value)} className="h-16 text-xl" />
              </div>
              <Button variant="premium" onClick={addToDb} className="w-full h-16 text-lg uppercase tracking-widest">Register Food</Button>
            </div>
          </div>
          <div className="sys-card p-10">
            <h3 className="text-xs font-black uppercase tracking-[0.3em] mb-10 opacity-30">Available Assets</h3>
            {foodDb.length === 0 ? <div className="py-24 text-center text-ink/10 text-sm font-black uppercase tracking-widest border-2 border-dashed border-ink/5">Registry Empty</div> : (
              <div className="space-y-4">
                {foodDb.map(food => (
                  <div key={food.id} className="flex justify-between items-center p-6 bg-paper border-2 border-transparent group hover:border-forest/30 transition-all">
                    <div><div className="text-xl font-black text-ink">{food.name}</div><div className="text-xs font-black opacity-20 uppercase tracking-[0.2em]">{food.calories} kcal / {food.baseWeight}g</div></div>
                    <button onClick={() => deleteFromDb(food.id)} className="opacity-0 group-hover:opacity-100 text-rust transition-all p-3 hover:bg-rust/10"><HugeiconsIcon icon={Delete02Icon} size={22} /></button>
                  </div>
                ))}
              </div>
            )}
       ...
       {tab === 'workout' && (
        <div className="animate-scale-in space-y-8">
           <div className={`sys-card p-12 text-center transition-all duration-500 ${isMissed ? 'bg-rust/10 border-rust border-l-8' : isWorkoutDay && workoutLoggedToday ? 'bg-forest/10 border-forest border-l-8' : 'bg-paper-dark border-ink/5'}`}>
              {isMissed ? (
                <HugeiconsIcon icon={ZapIcon} size={48} className="text-rust mx-auto mb-8 animate-bounce" />
              ) : (
                <HugeiconsIcon icon={Dumbbell01Icon} size={48} className={`mx-auto mb-8 ${isWorkoutDay && workoutLoggedToday ? 'text-forest' : 'text-ink/20'}`} />
              )}
              
              <h3 className="text-4xl font-black text-ink mb-4 tracking-tighter">
              ...
              <div className="flex justify-center gap-4 mb-8">
                 {days.map((d, i) => {
                   ...
                   return (
                     <div key={d} className={`text-[10px] font-black px-3 py-1 rounded-full ${isW ? (i === todayIdx ? 'bg-forest text-paper' : 'border-2 border-forest text-forest') : 'opacity-20'}`}>
                       {d.toUpperCase()}
                     </div>
                   );
                 })}
              ...
              {isMissed ? (
                <p className="text-sm text-rust font-black uppercase tracking-widest">Excuses don't burn calories. Log it now.</p>
              ) : isWorkoutDay && workoutLoggedToday ? (
                <p className="text-sm text-forest font-black uppercase tracking-widest">Performance optimized for today.</p>
              ) : (
                <p className="text-sm text-ink/40 font-bold max-w-xs mx-auto leading-relaxed">
           ...
           <div className="sys-card p-10"><div className="flex gap-6 items-end"><div className="flex-1"><Input label="Duration (Minutes)" type="number" placeholder="60" value={wDuration} onChange={e => setWDuration(e.target.value)} className="h-16 text-xl" /></div><Button variant="premium" onClick={addWorkout} className="h-16 px-12 text-lg">Log</Button></div></div>
           <div className="space-y-6">
              {workouts.map((w) => (
                <div key={w.id} className="sys-card p-8 flex justify-between items-center border-l-8 border-l-forest hover:translate-x-2 transition-all group">
                   <div><div className="text-xs font-black opacity-20 uppercase tracking-widest mb-2">{w.date} · {w.time}</div><div className="text-2xl font-black text-ink tracking-tighter">Session Complete</div></div>
                   <div className="flex items-center gap-8">
                      <div className="text-4xl font-black text-forest tracking-tighter">{w.duration}<span className="text-xs ml-2 opacity-30 uppercase font-black">min</span></div>
                      <button onClick={() => deleteWorkout(w.id)} className="opacity-0 group-hover:opacity-100 text-rust p-3 hover:bg-rust/10 transition-all"><HugeiconsIcon icon={Delete02Icon} size={22} /></button>
                   </div>
                </div>
              ))}
           </div>
              ...
              <div className="h-[350px] w-full">
                <ResponsiveContainer>
                  <BarChart data={weekCalories} margin={{ top: 10, right: 0, left: -20, bottom: 0 }}>
                    ...
                    <XAxis dataKey="day" tick={{ fill: 'var(--ink)', opacity: 0.3, fontSize: 12, fontWeight: 900 }} axisLine={false} tickLine={false} dy={15} />
                    <YAxis tick={{ fill: 'var(--ink)', opacity: 0.3, fontSize: 12, fontWeight: 900 }} axisLine={false} tickLine={false} />
